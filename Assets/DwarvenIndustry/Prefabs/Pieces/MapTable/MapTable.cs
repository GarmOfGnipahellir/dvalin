...
namespace Dvalin
{
    public class MapTable : MonoBehaviour
    {
        ...
        [HideInInspector] public Texture2D m_forestMaskTexture;
        [HideInInspector] public Texture2D m_mapTexture;
        [HideInInspector] public Texture2D m_heightTexture;
        [HideInInspector] public Texture2D m_fogTexture;
        [HideInInspector] public bool[] m_explored;

        [HideInInspector] public ZNetView m_nview;
        ...
        void Start()
        {
            ...
            if (!m_nview)
            {
                Logger.LogError("No ZNetView found on map table.");
                return;
            }
        ...
        bool UpdateMap(Switch sw, Humanoid user, ItemDrop.ItemData item)
        {
            if (item != null)
            {
                Logger.LogInfo("Items are not usuable on map table.");
                return false;
            }
            ...
            if (!player)
            {
                Logger.LogInfo("Map table used by non-player humanoid.");
                return false;
            }

            if (!TryCombineExploration())
            {
                Logger.LogWarning("Couldn't combine eploration.");
                return false;
            }
        ...
        bool TryCombineExploration()
        {
            ...
            {
                Logger.LogError("Eploration array sizes doesn't match.");
            }
